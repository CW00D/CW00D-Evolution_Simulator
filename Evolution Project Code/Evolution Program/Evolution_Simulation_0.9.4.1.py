...
class Simulation_Screen():
    ...
    def position_veg_and_fruit(self):
        ...
        for i in range(round(self.simulation_map.number_of_food_sources*proportion_of_veg)):
            ...
            if random.randint(0,10) <= 5:
                ...
                veg_item = Food(food_type="Veg", nutritional_value="ccdccaaadc", consumption_code="abbcabcbdb", x_position=x, y_position=y, map_id=self.simulation_map.id)
                session.add(veg_item)
            else:
                tile_at_xy.tile_type = "Fruit"
                fruit_item = Food(food_type="Fruit", nutritional_value="ccbccaaabc", consumption_code = "accbacbcdc", x_position=x, y_position=y, map_id=self.simulation_map.id)
    ...
    def position_meat(self):
        ...
        for i in range(round(self.simulation_map.number_of_food_sources*proportion_of_meat)):
            ...
            #ddbddabadc
            meat_item = Food(food_type="Meat", nutritional_value="a", consumption_code="cdccdddccd", x_position=x, y_position=y, map_id=self.simulation_map.id)
    ...
    def create_species(self):#add new chromosome structures + change the way the reproduction code is created
        standard_chromosomes = ["abbaca,aabab|b##db,abc|abccc","bbadbc,abaa|a,adc|ac,ac"]
        for species in range(self.simulation.starting_number_of_species):
            ...
            new_species_reproduction_code = "a"
        ...
        for x in range(self.map_size):
            for y in range(self.map_size):
                ...
                if tile_at_xy.tile_type == "Water":
                    ...
                    self.tile_array[x][y] = Fruit_Tile(self.simulation_map, self.surface, self.simulation_surface, self.block_size, x*self.block_size, y*self.block_size, x, y, self.screen_width)
                    ...
                    self.tile_array[x][y] = Organism_Tile(self.simulation_map, self.surface, self.simulation_surface, self.block_size, x*self.block_size, y*self.block_size, x, y, self.screen_width)
    ...
    def move_up(self, organism_to_move):
        if organism_to_move.y_position > 0:
            if self.tile_array[organism_to_move.x_position][organism_to_move.y_position-1].tile_type == "Blank":
                tile_to_move_from = session.query(Tile).filter(Tile.x_coordinate==organism_to_move.x_position).filter(Tile.y_coordinate==organism_to_move.y_position).filter(Tile.simulation_id==self.simulation.id).one()
                organism_to_move.y_position -= 1
                tile_to_move_to = session.query(Tile).filter(Tile.x_coordinate==organism_to_move.x_position).filter(Tile.y_coordinate==organism_to_move.y_position).filter(Tile.simulation_id==self.simulation.id).one()
    ...
    def check_area_around(self, tile):
        for neighbour in tile.neighbouring_tiles:
            ...
            if neighbour.tile_type == "Organism":
                if neighbour.organism_on_tile.reproduction_code == tile.organism_on_tile.reproduction_code:
                ...
                action_performed = True
            elif neighbour.tile_type == "Water" and action_performed == True:
                action_performed = True
                print("Drink water")
    ...
    def eat_food(self, organism, food):
        if food.food_type == "Meat":
            ...
            organism_consumer_code = list(organism.chromosome.split("|")[1].split(",")[1])
            total = 0
            ...
            if total > 0:
                print("Eating Meat")
                print(organism.reservoir)
                print(food.nutritional_value)
...
class Veg_Tile(Simulation_Tile):
    def __init__(self, food_map, surface, simulation_surface, block_size, x_position, y_position, x_coordinate, y_coordinate, screen_width):
        super().__init__(surface, simulation_surface, block_size, x_position, y_position, x_coordinate, y_coordinate, screen_width)
        self.food_on_tile = session.query(Food).filter(Food.x_position==x_coordinate).filter(Food.y_position==y_coordinate).filter(Food.map_id==food_map.id).one()
...
class Organism_Tile(Simulation_Tile):
    def __init__(self, simulation, surface, simulation_surface, block_size, x_position, y_position, x_coordinate, y_coordinate, screen_width):
        super().__init__(surface, simulation_surface, block_size, x_position, y_position, x_coordinate, y_coordinate, screen_width)
        self.organism_on_tile = session.query(Organism).filter(Organism.x_position==x_coordinate).filter(Organism.y_position==y_coordinate).filter(Organism.simulation_id==simulation.id).one()
