...
class Food(Base):
    ...
    def __repr__(self):
        return "<Food(food id: {0}, food type: {1}, nutritional value: {2}, map id: {3}, organism id: {4})>".format(self.id, self.food_type, self.nutritional_value, self.map_id, self.organism_id)
    ...
    if session.query(Food).count() == 0:
            with open("foods.csv", "r") as foods_file:
                ...
                for line in lines:
                    ...
                    new_food = Food(food_type=food_type, nutritional_value=nutritional_value, map_id=map_id, organism_id=organism_id)
