...
class Simulation_Screen():
    ...
    def create_species(self):#add new chromosome structures + change the way the reproduction code is created
        standard_chromosomes = ["abbaca,aabab|b##db,aaaa,abc|abccc","bbadbc,bbbb,abaa|a,bbbb,adc|ac,ac"]
    ...
    def move_up(self, organism_to_move):
        if organism_to_move.y_position > 0:
            if self.tile_array[organism_to_move.x_position][organism_to_move.y_position-1].tile_type=="Blank":
                #print(organism_to_move.tile_relationship)
    ...
    def check_area_around(self, tile):#modify
        for neighbour in tile.neighbouring_tiles:
            action_performed = False
            if neighbour.tile_type == "Organism":
                if neighbour.organism_on_tile.species_relationship == tile.organism_on_tile.species_relationship:
    ...
    def attack(self, organism_1, organism_2):
        print(organism_1, "fighting", organism_2)
        value = 0
    ...
    def convert_to_meat(self, organism_to_convert):
        ...
        tile = session.query(Tile).filter(Tile.x_coordinate==organism_to_convert.x_position).filter(Tile.y_coordinate==organism_to_convert.y_position).filter(Tile.simulation_relationship==self.simulation).one()
        print(organism_to_convert)
...
class Meat_Tile(Simulation_Tile):
    def __init__(self, food_map, surface, simulation_surface, block_size, x_position, y_position, x_coordinate, y_coordinate, screen_width):
        ...
        try:    
            ...
            print(session.query(Food).filter(Food.x_coordinate==x_coordinate).filter(Food.y_coordinate==y_coordinate).filter(Food.map_id==food_map.id).all())
