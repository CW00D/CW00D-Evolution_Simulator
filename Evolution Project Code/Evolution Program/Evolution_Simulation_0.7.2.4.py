...
class New_simulation_screen():
    ...
    def randomly_generate(self):
        ...
        temperature_range = random.randint(0, 5)
        ...
        map_to_add = Map(map_size=map_size, number_of_food_sources=number_of_food_sources, meat_to_veg_ratio=meat_to_veg_ratio, number_of_water_sources=number_of_water_sources, average_temperature=average_temperature, temperature_range=temperature_range, simulation_id=simulation.id, simulation_relationship=simulation)
...
class Custom_Detail_Entry_screen():
    ...
    def collect_input_data(self):
        ...
        self.generate(simulation_name, date_created, length_of_turn, starting_number_of_organisms, starting_number_of_species, map_size, number_of_food_sources, meat_to_veg_ratio, number_of_water_sources, average_temperature, temperature_range)
    
    def generate(self, simulation_name, date_created, lenth_of_turn, starting_number_of_organisms, starting_number_of_species, map_size, number_of_food_sources, meat_to_veg_ratio, number_of_water_sources, average_temperature, temperature_range):
...
class Simulation_Screen():
    ...
    def position_veg(self):
        ...
        for i in range(int(self.simulation_map.number_of_food_sources*proportion_of_veg)):
            x = random.randint(0, self.map_size-1)
            y = random.randint(0, self.map_size-1)
            tile_at_xy = session.query(Tile).filter(Tile.x_coordinate == x).filter(Tile.y_coordinate == y).filter(Tile.simulation_relationship == Program.current_simulation).one()
            while tile_at_xy.tile_type != "Blank":
    ...
    def position_meat(self):
        ...
        for i in range(int(self.simulation_map.number_of_food_sources*proportion_of_meat)):
            x = random.randint(0, self.map_size-1)
            y = random.randint(0, self.map_size-1)
            tile_at_xy = session.query(Tile).filter(Tile.x_coordinate == x).filter(Tile.y_coordinate == y).filter(Tile.simulation_relationship == Program.current_simulation).one()
            while tile_at_xy.tile_type != "Blank":
