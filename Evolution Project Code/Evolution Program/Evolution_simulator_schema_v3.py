...
class Organism(Base):
    ...
    tile_relationship = relationship("Tile", back_populates='organism_relationship')

    def __repr__(self):
        return "<Organism(id: {0}, chromosome: {1}, reservoir: {2}, colour: {3}, reproduction_code: {4}, position: ({5}:{6}))>".format(self.id, self.chromosome, self.reservoir, self.colour, self.reproduction_code, self.x_position, self.y_position)    
...
class Map(Base):
    ...
    meat_to_veg_ratio = Column(String(10))
...
class Food(Base):
    ...
    tile_relationship = relationship("Tile", back_populates='food_relationship')


    def __repr__(self):
        return "<Food(food id: {0}, food type: {1}, nutritional value: {2}, map id: {3})>".format(self.id, self.food_type, self.nutritional_value, self.map_id)
