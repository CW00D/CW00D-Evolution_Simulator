...
from Evolution_simulator_schema_v6 import User, Simulation, Species, Species_History, Organism, Map, Food, Tile
...
class Custom_Detail_Entry_screen():
    def __init__(self):
        ...
        self.starting_number_of_organisms = self.menu.add_text_input("Starting Number of Organisms:  ", default="20")
        self.starting_number_of_species = self.menu.add_text_input("Starting Number of Species:  ", default="4")
        self.menu.add_label("")
        self.menu.add_label("Map Details")
        self.map_size = self.menu.add_text_input("Map Size:  ", default="10")
        self.number_of_food_sources = self.menu.add_text_input("Number of Food Sources:  ", default="20")
        self.meat_to_veg_ratio = self.menu.add_text_input("Meat to Veg Ratio:  ", default="1:4")
        self.number_of_water_sources = self.menu.add_text_input("Number of Water Sources:  ", default="10")
        self.menu.add_label("")
    ...
    def collect_input_data(self):
        ...
        if self.validate_data(length_of_turn, starting_number_of_organisms, starting_number_of_species, map_size, number_of_food_sources, meat_to_veg_ratio, number_of_water_sources):
    ...
    def validate_data(self, length_of_turn, starting_number_of_organisms, starting_number_of_species, map_size, number_of_food_sources, meat_to_veg_ratio, number_of_water_sources):
        try:
            int(length_of_turn)
            ...
            return False
        if int(length_of_turn) <= 0 or int(starting_number_of_organisms) <= 0 or int(starting_number_of_species) <= 0 or int(map_size) <= 0 or int(number_of_food_sources) < 0 or int(number_of_water_sources) < 0:
            return False
        ...
        if int(starting_number_of_species) > int(starting_number_of_organisms):
            return False
        if int(starting_number_of_organisms) + int(number_of_food_sources) + int(number_of_water_sources) > int(map_size)**2:
            return False
        return True
...
class Simulation_Screen():
    def __init__(self):
        ...
        if len(self.simulation.tile_relationship) == 0:
            ...
            for species in session.query(Species).filter(Species.simulation_relationship==self.simulation).all():
                new_species_history = Species_History(turn_number = self.simulation.current_turn, number_of_organisms=len(species.organism_relationship), average_age_of_organisms=0, average_variance_from_originals=0, species_relationship=species, species_id=species.id)
        ...
        while True:
            ...
            if self.simulation_state == 1:
                ...
                for tile in self.organism_tiles:
                    tile.organism_on_tile.reservoir = tile.organism_on_tile.reservoir[tile.organism_on_tile.age:]
                    session.commit()
    ...
    def position_veg_and_fruit(self):
        ...
        for i in range(round(self.simulation_map.number_of_food_sources*proportion_of_veg)):
            ...
            if random.randint(0,10) <= 5:
                ...
                veg_item = Food(food_type="Veg", nutritional_value="acdacadadcacdacadadcacdacadadc", consumption_code="abbcabcbdb", x_coordinate=x, y_coordinate=y, map_id=self.simulation_map.id, map_relationship=self.simulation_map, tile_relationship=tile_at_xy)
                session.add(veg_item)
                tile_at_xy.food_relationship=veg_item
                tile_at_xy.food_id=veg_item.id
            else:
                tile_at_xy.tile_type = "Fruit"
                fruit_item = Food(food_type="Fruit", nutritional_value="cabdcababccabdcababccabdcababc", consumption_code = "addcacbcdc", x_coordinate=x, y_coordinate=y, map_id=self.simulation_map.id, map_relationship=self.simulation_map, tile_relationship=tile_at_xy)
    ...
    def create_species(self):
        standard_chromosomes = [".+z5","abadcc,bbbb|a,bbbb,abdc|ac,ac"]
    ...
    def create_organisms(self):
        ...
        for current_species in species:
            for i in range(int(self.simulation.starting_number_of_organisms/self.simulation.starting_number_of_species)):
                ...
                new_organism = Organism(chromosome=current_species.average_chromosome, reservoir="abcd", colour=current_species.average_species_colour, reproduction_code=current_species.average_reproduction_code, x_position=x, y_position=y, age=0, species_id=current_species.id, species_relationship=current_species, simulation_id=self.simulation.id, simulation_relationship=self.simulation)
    ...
    def convert_to_meat(self, organism_to_convert):
        ...
        meat_tile = Meat_Tile(self.simulation_map, self.surface, self.simulation_surface, self.block_size, organism_to_convert.x_position*self.block_size, organism_to_convert.y_position*self.block_size, organism_to_convert.x_position, organism_to_convert.y_position, self.screen_width)
        
        neighbours = self.tile_array[organism_to_convert.x_position][organism_to_convert.y_position].neighbouring_tiles
        for neighbour in neighbours:
            neighbour.neighbouring_tiles[neighbour.neighbouring_tiles.index(self.tile_array[organism_to_convert.x_position][organism_to_convert.y_position])] = meat_tile
        self.tile_array[organism_to_convert.x_position][organism_to_convert.y_position] = meat_tile
        self.tile_array[organism_to_convert.x_position][organism_to_convert.y_position].tile_type = "Meat"
        self.tile_array[organism_to_convert.x_position][organism_to_convert.y_position].neighbouring_tiles = neighbours
...
class Graph_Screen():
        ...
        while True:
            ...
            for event in pygame.event.get(): 
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if self.quit_button_position[0] <= mouse[0] <= self.quit_button_position[0]+140 and self.quit_button_position[1] <= mouse[1] <= self.quit_button_position[1]+40: 
                        self.quit_graphing()  
                    if self.graph_1_button_position[0] <= mouse[0] <= self.graph_1_button_position[0]+1180 and self.graph_1_button_position[1] <= mouse[1] <= self.graph_1_button_position[1]+40: 
                        self.current_graph = 1
                        self.draw_graph()        
                    if self.graph_2_button_position[0] <= mouse[0] <= self.graph_2_button_position[0]+180 and self.graph_2_button_position[1] <= mouse[1] <= self.graph_2_button_position[1]+40: 
                        self.current_graph = 2
                        self.draw_graph()        
                    if self.graph_3_button_position[0] <= mouse[0] <= self.graph_3_button_position[0]+200 and self.graph_3_button_position[1] <= mouse[1] <= self.graph_3_button_position[1]+40: 
        ...
        if self.current_graph == 1:
            ...
            for species in range(number_of_species):
                ...
                for turn in range(Program.current_simulation.current_turn+1):
                    species_history_item = session.query(Species_History).filter(Species_History.species_relationship == Program.current_simulation.species_relationship[species]).filter(Species_History.turn_number == turn).one()
    ...
    def draw_buttons(self):
        ...
        pygame.draw.rect(self.surface,WHITE,[self.graph_1_button_position[0],self.graph_1_button_position[1],180,40],1) 
        self.surface.blit(self.graph_1_text , (self.graph_1_button_position[0]+30,self.graph_1_button_position[1]))

        pygame.draw.rect(self.surface,WHITE,[self.graph_2_button_position[0],self.graph_2_button_position[1],180,40],1) 
        self.surface.blit(self.graph_2_text , (self.graph_2_button_position[0]+30,self.graph_2_button_position[1]))

        pygame.draw.rect(self.surface,WHITE,[self.graph_3_button_position[0],self.graph_3_button_position[1],200,40],1) 
        self.surface.blit(self.graph_3_text , (self.graph_3_button_position[0]+30,self.graph_3_button_position[1]))
