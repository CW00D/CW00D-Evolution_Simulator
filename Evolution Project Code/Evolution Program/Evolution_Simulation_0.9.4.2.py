...
class Simulation_Screen():
    def __init__(self):
        ...
        if len(self.simulation.tile_relationship) == 0:
            self.set_up_grid()#works
            self.position_water()#works
            self.position_veg_and_fruit()#works
            self.position_meat()#works
            self.create_species()#works
    ...
    def create_organisms(self):
        ...
        for current_species in species:
            for i in range(int(self.simulation.starting_number_of_organisms/self.simulation.starting_number_of_species)):
                ...
                new_organism = Organism(chromosome=current_species.average_chromosome, reservoir="", colour=current_species.average_species_colour, reproduction_code=current_species.average_reproduction_code, x_position=x, y_position=y, species_id=current_species.id, species_relationship=current_species, simulation_id=self.simulation.id, simulation_relationship=self.simulation)
