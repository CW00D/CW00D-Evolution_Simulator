    ...
    def set_up_grid(self):
        for x in range(self.map_size):
            for y in range(self.map_size):
                tile = Tile(x_coordinate=x, y_coordinate=y, x_position=x*self.block_size, y_position=y*self.block_size, tile_type="Blank", simulation_relationship=self.simulation)
                session.add(tile)
                session.commit()
