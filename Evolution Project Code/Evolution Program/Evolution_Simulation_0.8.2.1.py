...
class Simulation_Screen():
    ...
    def create_organisms(self):
        species = session.query(Species).filter(Species.simulation_relationship == Program.current_simulation).all()
        for current_species in species:
            organisms_of_species = int(self.simulation.starting_number_of_organisms/self.simulation.starting_number_of_species)
            for organism in range(organisms_of_species):
                chromosome = current_species.average_chromosome
                tag_section = current_species.average_chromosome.split("|")[0]
                control_section = current_species.average_chromosome.split("|")[1]
                exchange_section = current_species.average_chromosome.split("|")[2]
                reservoir = ""
                reproduction_code = current_species.average_reproduction_code
                species_colour = self.find_colour(tag_section, control_section, exchange_section)
                species_id = current_species.id
                position_x = random.randint(0, self.map_size -1)
                position_y = random.randint(0, self.map_size -1)
                tile_at_xy = session.query(Tile).filter(Tile.x_coordinate == position_x).filter(Tile.y_coordinate == position_y).filter(Tile.simulation_relationship == Program.current_simulation).one()
                while tile_at_xy.tile_type != "Blank":
                    x = random.randint(0, self.map_size-1)
                    y = random.randint(0, self.map_size-1)
                    tile_at_xy = session.query(Tile).filter(Tile.x_coordinate == x).filter(Tile.y_coordinate == y).filter(Tile.simulation_relationship == Program.current_simulation).one()
                tile_at_xy.tile_type = "Organism"
                organism = Organism(chromosome=chromosome, reservoir=reservoir, reproduction_code=reproduction_code, x_position=position_x, y_position=position_y, species_id=species_id, simulation_relationship=self.simulation)
        ...
        for x in range(self.map_size):
            for y in range(self.map_size):
                ...
                if tile_at_xy.tile_type == "Blank":
                    ...
                    self.tile_array[x][y] = Water_Tile(self.surface, self.simulation_surface, self.block_size, x*self.block_size, y*self.block_size, self.screen_width)
                elif tile_at_xy.tile_type == "Veg":
                    self.tile_array[x][y] = Veg_Tile(self.surface, self.simulation_surface, self.block_size, x*self.block_size, y*self.block_size, self.screen_width)
                elif tile_at_xy.tile_type == "Meat":
                    self.tile_array[x][y] = Meat_Tile(self.surface, self.simulation_surface, self.block_size, x*self.block_size, y*self.block_size, self.screen_width)
                elif tile_at_xy.tile_type == "Organism":
                    organism_at_xy = session.query(Organism).filter(Organism.x_position==x).filter(Organism.y_position==y).filter(Organism.simulation_relationship==self.simulation).one()
                    print(organism_at_xy.colour)
                    self.tile_array[x][y] = Organism_Tile(self.surface, self.simulation_surface, self.block_size, x*self.block_size, y*self.block_size, self.screen_width)
