...
class Simulation_Screen():
    ...
    def position_veg_and_fruit(self):
        ...
        for i in range(round(self.simulation_map.number_of_food_sources*proportion_of_veg)):
            ...
            if random.randint(0,10) <= 5:
                ...
                veg_item = Food(food_type="Veg", nutritional_value="acdacadadc", consumption_code="abbcabcbdb", x_coordinate=x, y_coordinate=y, map_id=self.simulation_map.id, map_relationship=self.simulation_map, tile_relationship=tile_at_xy)
                session.add(veg_item)
                tile_at_xy.food_relationship=veg_item
                tile_at_xy.food_id=veg_item.id
            else:
                tile_at_xy.tile_type = "Fruit"
                fruit_item = Food(food_type="Fruit", nutritional_value="cabdcababc", consumption_code = "accbacbcdc", x_coordinate=x, y_coordinate=y, map_id=self.simulation_map.id, map_relationship=self.simulation_map, tile_relationship=tile_at_xy)
    ...
    def end_turn(self):
        ...
        for tree in trees:
            tree.nutritional_value = "cabdcababc"
            tree.food_type = "Fruit"
            tree.tile_relationship.tile_type = "Fruit"
            session.commit()
        for fruit in fruits:
            fruit.nutritional_value = "cabdcababc"
            session.commit()
